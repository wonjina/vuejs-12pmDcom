...
export default {
  ...
  computed: {
    ...mapState([
      'newRecruitmentModalFlage', 'userInfo'
    ]),
    items () {
  ...
  methods: {
    ...
    addNewRecruitment () {
      urls.newRecruitment.data.memberId = this.userInfo.user.member_id
      urls.newRecruitment.data.restaurantId = this.restaurantId
      urls.newRecruitment.data.subject = this.subjectText
      urls.newRecruitment.data.maxNumber = this.maxNum

      restful
        .postRequest(urls.newRecruitment.method, urls.DOMAIN + urls.newRecruitment.path, urls.newRecruitment.data)
        .then(data => {
          swal({
            title: '등록되었습니다.',
            icon: 'success'
